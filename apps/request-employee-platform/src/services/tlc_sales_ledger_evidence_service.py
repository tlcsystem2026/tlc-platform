...
FIELD_CANDIDATES = {
    "id": ("id", "entry_id", "ledger_id", "request_id"),
    "customer_id": (
        "customer_id",
        "customer_code",
        "client_id",
...
def discover_sales_source(db: Session) -> dict[str, str]:
    for table_name in SALES_TABLE_CANDIDATES:
        ...
        if (
            ...
            and mapping["amount"]
            and (mapping["customer_id"] or mapping["customer_name"])
        ):
...
def list_sales_evidence(
    ...
    *,
    ...
    source = discover_sales_source(db)
    if not source:
        return {
            "source_table": "",
    ...
    rows = db.execute(
        text(
            ...
            f"ORDER BY {source['business_date']}, "
            f"{source.get('id') or source['business_date']} "
            "LIMIT :limit"
        ),
        params,
    ).all()
    ...
    return {
        ...
        "sales_total": _format_decimal(total),
        "records": records,
    }


def compare_snapshot_sales(
    db: Session,
    *,
    snapshot_id: str,
    limit: int = 1000,
    ...
    evidence = list_sales_evidence(
        db,
        customer_id=snapshot_data.get("customer_id", ""),
        customer_name=snapshot_data.get("customer_name", ""),
        previous_cutoff=snapshot_data["previous_request_cutoff"],
        current_cutoff=snapshot_data["current_request_cutoff"],
        limit=limit,
    )

    snapshot_total = _decimal(snapshot_data.get("sales_total"))
    evidence_total = _decimal(evidence.get("sales_total"))
    difference = snapshot_total - evidence_total

    return {
        "snapshot": snapshot_data,
        "evidence": evidence,
        "snapshot_sales_total": _format_decimal(snapshot_total),
        "evidence_sales_total": _format_decimal(evidence_total),
