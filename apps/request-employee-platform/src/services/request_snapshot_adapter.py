...
class RequestSnapshotAdapter:
 field_variants={'request_no':('request_no','請求書番号','请求书编号'),'customer_name':('customer_name','顧客名','客户名称'),'request_date':('request_date','請求日','请求日期'),'currency':('currency','通貨','币种'),'subtotal':('subtotal','税抜金額','小計'),'tax_amount':('tax_amount','消費税','税額'),'total_amount':('total_amount','税込金額','合計金額')}
 def from_dict(self,data,source_type,source_document_id=''):
  def pick(n,d=''):
   for k in self.field_variants[n]:
