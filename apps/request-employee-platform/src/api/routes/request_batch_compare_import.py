...
from src.services.request_batch_compare_import_service import latest_batch, list_review_queue, run_request_batch
...
def queue(business_month: str = "", limit: int = Query(default=1000, ge=1, le=2000), db: Session = Depends(get_db)):
    try:
        return list_review_queue(db, business_month, limit)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
